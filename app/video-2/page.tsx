...
export default function Video2Page() {
  ...
  useEffect(() => {
    ...
    const v1 = localStorage.getItem("video1Completed") === "true";
  ...
  useEffect(() => {
    if (!isReady || v2Completed) return;
    const interval = setInterval(() => {
      setPlayTimer((prev) => {
        if (prev >= VIDEO_2_PLAY_DURATION) {
          clearInterval(interval);
          return VIDEO_2_PLAY_DURATION;
        }
        return prev + 1;
      });
    }, 1000);
  ...
  const handleVideo2Complete = async () => {
    if (isSubmitting || v2Completed) return;
    setIsSubmitting(true);
    try {
      setV2Completed(true);
      localStorage.setItem("video2Completed", "true");
      toast({ title: "দারুণ!", description: "ভিডিও ২ সফলভাবে সম্পন্ন হয়েছে। পরবর্তী ভিডিও শুরু করুন।" });
        ...
        <div className="max-w-5xl mx-auto grid lg:grid-cols-3 gap-4 sm:gap-6 md:gap-8">
          ...
          <div className="lg:col-span-2 space-y-3 sm:space-y-4 md:space-y-6">
            <div className="space-y-1 sm:space-y-2 text-center md:text-left">
              <h1 className="text-lg sm:text-xl md:text-2xl lg:text-3xl font-black text-slate-800 leading-tight">পর্ব ০১: নির্বাচনী ও পোলিং এর নিয়োগের সঠিক পদ্ধতি</h1>
              <p className="text-xs sm:text-sm md:text-base text-slate-500">নির্বাচনী ও পোলিং এজেন্ট নিয়োগের সঠিক পদ্ধতি সম্পর্কে জানুন।</p>
            </div>
            ...
            <Card className="border-none shadow-2xl shadow-slate-200/50 overflow-hidden bg-black aspect-video relative">
              <iframe
                width="100%"
                height="100%"
                src={`https://www.youtube.com/embed/${VIDEO_2_ID}?autoplay=1&modestbranding=1&rel=0`}
            ...
            <div className="bg-white p-3 sm:p-4 md:p-6 rounded-xl sm:rounded-2xl shadow-sm border border-slate-100 flex flex-col sm:flex-row items-center justify-between gap-3 sm:gap-4 md:gap-6">
              <div className="flex items-center gap-2 sm:gap-3 md:gap-4 w-full sm:w-auto">
                <div className={cn(
                  ...
                  v2Completed ? "bg-emerald-50 text-emerald-600" : "bg-slate-50 text-slate-400"
                )}>
                  {v2Completed ? <CheckCircle2 className="w-4 h-4 sm:w-5 sm:h-5 md:w-6 md:h-6" /> : <Clock className="w-4 h-4 sm:w-5 sm:h-5 md:w-6 md:h-6" />}
                </div>
              ...
              {!v2Completed ? (
                <Button
                  onClick={handleVideo2Complete}
                  disabled={isSubmitting || playTimer < VIDEO_2_PLAY_DURATION}
                ...
                >
                  ...
                  ) : playTimer < VIDEO_2_PLAY_DURATION ? (
                    `অপেক্ষা করুন (${VIDEO_2_PLAY_DURATION - playTimer}s)`
                  ) : (
